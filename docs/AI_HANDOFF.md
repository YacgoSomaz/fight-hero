...
目标是研究如何把一个已获得本机运行权的 Flash 横版射击样本，逐步重建为浏览器 Canvas 验证程序。

> 本文的第 3–7 节主要保留迁移前的逆向过程。当前功能状态、验证结果和刻意保留的差异以 [MIGRATION_STATUS.md](MIGRATION_STATUS.md) 为准；其中“尚未接入”“后续推荐”的旧措辞不再表示当前范围。

最重要的限制：保留代码、测试、符号 ID、逆向笔记和本机提取说明即可。
...
3. 私有原始素材的符号索引与导出记录见 `PRIVATE_ASSET_SETUP.md`；当前授权素材、音频和参考帧已按私有仓库策略受版本控制。
4. 已迁移范围请先核对 `MIGRATION_STATUS.md`，再为尚未逐项复刻的职业、模式或导航网建立独立测试。
