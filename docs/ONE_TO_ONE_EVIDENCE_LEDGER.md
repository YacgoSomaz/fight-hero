...
| --- | --- | --- | --- | --- | --- |
| Campaign 1 `unit0` 的 root + M4/USP2 | `Stats_Campaign.setMatch()` 的 unit0/skin 7；`UnitMC` symbol 669/449 帧、`EnterFrame()`；完整 `Stats_Guns.as:addGun`；`Player.as:MouseDown/MouseUp/EnterFrame`；`Unit.as` aim/holder/flip；`Guns.as:shoot/EnterFrame`；501/668 Display List；`arm_gun_316.as` callback；`MuzzleFlash_317.as:gotoAndStop(UT.irand(1,totalFrames))` | `gun-source.mjs`、`tutorial-gun-runtime.mjs`、`tutorial-aim-runtime.mjs`、`tutorial-actor-bindings.mjs`、`tutorial-gun-action-frame.mjs`、`tutorial-actor-playback.mjs`、`tutorial-actor-preview.html`、`tutorial-actor-preview.mjs`；画布只读取原 Shape、root timeline 和原 arm runtime | `gun-source.test.mjs`、`tutorial-gun-runtime.test.mjs`、`tutorial-aim-runtime.test.mjs`、`tutorial-actor-render-plan.test.mjs`、`tutorial-actor-playback.test.mjs`、`tutorial-actor-preview.test.mjs`、`tutorial-usp2-action-frame.test.mjs`、`tutorial-unit-pose-plan.test.mjs`、`tutorial-unit-pose-renderer.test.mjs`、`usp2-muzzle-runtime-extraction.test.mjs` | 本地浏览器已加载 800×600 预览并检查无 console error；尚无原 SWF 同输入/同帧叠图 | `已接入+已回归（局部武器纵切）`；当前可由 Tutorial `Movement` 请求原 `run/jump/fall/land/climb` 标签，并可对 unit0 skin 57 回放 M4 与 Campaign 状态 8 的 USP2（pistol 2、fire 3–8、reload 9–37，USP Sprite frame 2）。USP2 现按原 `mDown`、non-auto `shotPressed`、`shootDelay=.25*30` 的 AS3 `uint` 写入及同 tick `Guns.EnterFrame` 递减、`noAmmo=true` 规则开火；鼠标经原 stage/Arena、half-step aim、holder/flip 关系进入 arm/head pose，首发 tick 保留 394 的一次原 1..8 随机帧和原 arm 局部矩阵。仍缺原 Aimer 1431 可见承载、`makeBullet`、弹道/伤害/音频、完整碰撞分支、AI、其它武器/动作及原版像素对照，故不可称可玩或完成。 |
| Campaign 1 Tutorial 场景 | `Arena.as:EnterFrame()` 0.7 跟随、边界钳制、背景视差式；`Stats_Campaign.runScripts` 的 frame 0 武器清空和脚底状态；状态 8 的 `USP2/noAim=false`；`Player.as:MouseDown/mDown`；`Movement.as` 的 alpha hitTest、±17/-45 自动起身探针、落地、左右十像素斜坡扫描与 terminal-fall climb；`Wall_tut` 16 帧/2757×1541；`Stats_Maps` 的 `tut` sky/background/Arena frame；Campaign 1 出生点 `{285,705}` | `tutorial-scene-preview.html`、`tutorial-scene-preview.mjs`、`campaign-one-session.mjs`、`tutorial-arena-camera.mjs`、`tutorial-movement.mjs`、`tutorial-gun-runtime.mjs`；每 source tick 消费 runScripts、每脚底 probe 消费 session wall trigger，A/D/S/W 输入进入当前原 `Wall_tut` surface；source 已授予 USP2 后，canvas 左键只置 `mDown`，随后由原式射击门槛决定是否进入 `pistol_fire` 和 394 Display List；nextAnim 进入 `UnitMC.goto` 规则 | `campaign-one-session.test.mjs`、`tutorial-arena-camera.test.mjs`、`tutorial-movement.test.mjs`、`tutorial-actor-playback.test.mjs`、`tutorial-scene-preview.test.mjs`、`tutorial-gun-runtime.test.mjs`、`tutorial-usp2-action-frame.test.mjs`、`usp2-muzzle-runtime-extraction.test.mjs`、既有 `tutorial-world*.test.mjs` | 本地浏览器实际加载 800×600 场景、原地图三层和 unit0；页面状态 ready 且无 error 文本；尚无原 SWF 同输入/同帧叠图 | `已接入+已回归（Movement/教程脚本/USP2 输入门控和首发显示列表核心纵切）`；已验证原 frame 0 装备状态进入 session、自动起身、贴墙蹲伏、落地、手动跳跃、侧墙小攀爬与左右十像素探针斜坡角，并验证状态 8 的 USP2 切枪和 `mDown→shootDelay/shotPressed/noAmmo`。完整 modifier/降落伞/硬着陆、完整大小攀爬、真实 mouse aim、`makeBullet`/命中/状态 9、伤害/音频、AI、HUD、过场及原版截图差分均尚未完成，不能说第一关或游戏完成。 |
