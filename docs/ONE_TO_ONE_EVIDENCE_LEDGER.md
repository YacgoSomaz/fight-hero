...
| 头顶血条 / 底部 HUD | `Hud` symbol 1540、`Status`、`Unit` symbol 687 的 `bar_hp/icon/bar_hurt/txt_name/txt_level` Display List、symbol 682、686、DefineEditText 684/685、DefineFont3 683 及 team ColorTransform | `main.mjs` 直接载入 `hud-scorebar-1462.png` 与 `hud-expholder-1477.png`，并以 `hud-ammo.mjs` 重建 954 的 `drawBox` 逻辑；`tutorial-unit-overhead-hud.mjs`、`tutorial-unit-overhead-icon.mjs`、`tutorial-unit-jug-marker.mjs`、`tutorial-unit-overhead-labels.mjs` 与 `tutorial-scene-preview.mjs` 直接载入 670/682/683/686，并使用 symbol 687 两帧的矩阵、`Status.barHpWidth`、`unitInfo.icon/name/level`、`isJug` 和 `Unit.setTeam` 色变换 | `unit-status.test.mjs`、`tutorial-unit-overhead-hud.test.mjs`、`tutorial-unit-overhead-icon.test.mjs`、`tutorial-unit-jug-marker.test.mjs`、`tutorial-unit-overhead-labels.test.mjs`、`tutorial-scene-preview.test.mjs`、`aimer-source.test.mjs`、`hud-ammo.test.mjs` | `部分接入+自动回归`；Tutorial 已接入原 670 的 HP/受伤条、原 682 的职业图标、原 683 字体的名字/等级及原 686 Jug 标记，非手画矩形或系统字体；1462/1477 已在原锚点接入，954 已按原公式和双轴翻转接入 M4 `arifle`。仍缺顶部生命区、底部职业/生命/备用弹文字、全部枪图、动态经验宽度和原版同帧截图，不能称 HUD 完成。 |
