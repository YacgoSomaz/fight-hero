...
- 30 个战役/挑战 `Stats_Campaign.setMatch()` 配置已由原 AS3 机械提取为浏览器目录数据；Campaign 1 的 9 个计时动作、4 个比分推进、14 个脚底状态、子弹环境命中与换枪推进也已从四个原始 AS3 类生成可审计数据，并组成保留原 Tutorial Arena/5 个演员记录的独立会话模型。Tutorial 原 `Wall_tut` symbol 1378 的 16 帧亦已随运行时资源交付并完成触发色审计；尚未迁移完整角色、过场和结算的任务会明确拒绝启动，绝不回退为快速对战；
- Tutorial `Wall_tut` 运行时加载器现要求原始 16 帧全部成功加载，并只允许按原 SWF 帧号选择已解码的碰撞蒙版。独立 `tutorial-scene-preview.html` 已把当前 wall surface 接入 `Movement.as` 的核心 A/D/S/W 探针、落地与侧墙攀爬分支、UnitMC 原标签转场，以及 `Stats_Campaign.runScripts` 的逐帧/脚底触发状态；原第 0 帧武器清空与后续 `noAim/noJump/墙体帧` 会更新场景状态。状态 8 的 `USP2` 已从同一原 501/668 arm Display List 读取 `pistol/pistol_fire/pistol_reload` 和枪 Sprite 第 2 帧；`MuzzleFlash_317`（394）的原 8 帧 Shape/Display List 及其原 `UT.irand(1,totalFrames)` 选择现已直接提取。`src/gun-source.mjs` 机械携带全部 `Stats_Guns.addGun` 记录，Tutorial 中的 `Player.mDown → Guns.shoot → shootDelay/shotPressed/noAmmo` 已按 `USP2` 的原规则逐 tick 回放，并保留开火 tick 的首帧枪口火焰。鼠标现按 Canvas→原 800×600 舞台→Arena local 的关系进入 `Player.aimX/aimY` 半步平滑、`Unit` 的 arm/head holder 旋转与整身翻转。**它仍缺 `makeBullet`/命中/状态 9、原 Aimer 可见组件、声音、HUD、AI 及原版逐帧对照，不能开放战役入口；**
- `npm run test:coverage` 当前为 **230/230 通过**（99.01% 行、82.79% 分支、95.96% 函数）。其中包含战役目录与 Campaign 1 脚本源码一致性、Hud 文本字段和 Aimer Flash 矩阵→Canvas 映射回归，以及 UnitMC `setSkin()` 嵌套子部件、完整根时间轴标签/转场/端点播放、`EnterFrame()` 的 head/arm holder 重定位、直接 Shape 裁切原点、M4/USP2 手臂的真实离散动作帧/回调、Campaign 1 `tut` 原地图三层/出生坐标/原 Arena 相机、Movement 核心与逐帧/脚底脚本浏览器承载；这些都不等于可玩战役、动态经验条或完整 HUD 已完成。
