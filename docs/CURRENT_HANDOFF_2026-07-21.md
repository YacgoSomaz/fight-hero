...
本地入口：<http://127.0.0.1:4173>。当前基线：`npm test` **117/117 通过**，覆盖率为 **99.07% 行、88.03% 分支、93.52% 函数**，高于 80% 门槛。
...
**未完成且不能误报**：`Player.as` 的动态 recoil 会独立移动四条线并缩放 circle；当前只接入 26×26 的原静态合成帧。下一位开发者应先将 1424/1428 逐一导出并按 `Player.as` 公式组装，再处理完整 Hud 1540。还必须先修正原 SWF 800×600 与现有 1280×720 的坐标/纵横比契约，才可做像素级 HUD 对照。
