...
| 头顶血条 / 底部 HUD | `Hud` symbol 1540、`Hud.as:addExp`、`Status`、`Unit` symbol 687 的 `bar_hp/icon/bar_hurt/txt_name/txt_level` Display List、symbol 682、686、DefineEditText 684/685、DefineFont3 683 及 team ColorTransform | `main.mjs` 直接载入 `hud-scorebar-1462.png`，并由 `hud-experience-render-plan.mjs` 以 1474/1475/699 的原 child Display List、矩阵和颜色变换动态绘制 `expholder`；`hud-ammo.mjs` 重建 954 的 `drawBox` 逻辑；`tutorial-unit-overhead-hud.mjs`、`tutorial-unit-overhead-icon.mjs`、`tutorial-unit-jug-marker.mjs`、`tutorial-unit-overhead-labels.mjs` 与 `tutorial-scene-preview.mjs` 直接载入 670/682/683/686，并使用 symbol 687 两帧的矩阵、`Status.barHpWidth`、`unitInfo.icon/name/level`、`isJug` 和 `Unit.setTeam` 色变换 | `unit-status.test.mjs`、`tutorial-unit-overhead-hud.test.mjs`、`tutorial-unit-overhead-icon.test.mjs`、`tutorial-unit-jug-marker.test.mjs`、`tutorial-unit-overhead-labels.test.mjs`、`tutorial-scene-preview.test.mjs`、`aimer-source.test.mjs`、`hud-ammo.test.mjs`、`hud-experience-render-plan.test.mjs`、`hud-experience-runtime.test.mjs` | `部分接入+自动回归`；Tutorial 已接入原 670 的 HP/受伤条、原 682 的职业图标、原 683 字体的名字/等级及原 686 Jug 标记，非手画矩形或系统字体；1462/954/M4 724 已接入，1477 已替换为其原 1474/1475/699 动态子树。仍缺顶部生命区、底部职业/生命/备用弹文字、全部枪图、来自原对局的经验状态和原版同帧截图，不能称 HUD 完成。 |
...
| Campaign 1 Unit/Status/Bullet 数值状态 | `Stats_Skills.Init` 22 条 `addSkill`；`Stats_Classes.getAiLevel/getClass`；`Unit.setClass/unitSpawn`；`Player.spawn/AI.spawn`；`Status.reset/damage/EnterFrame`；`Bullet_Line_Basic → Bullet.doHitEffect` | `skill-source.mjs`、`tutorial-unit-profile.mjs`、`tutorial-status-damage-runtime.mjs`、`tutorial-bullet-hit-effects.mjs`、`campaign-one-session.mjs`；已 spawn actor 有 source `unitInfo/status/gun`，page USP2 命中会调用 `Status.damage` 并在其后推进 `Status.EnterFrame`，dead actor 会按来源 node 重生 | `skill-source.test.mjs`、`tutorial-unit-profile.test.mjs`、`tutorial-status-damage-runtime.test.mjs`、`tutorial-bullet-hit-effects.test.mjs`、`campaign-one-session.test.mjs`、`tutorial-scene-preview.test.mjs` | 仅验证页面无加载错误；尚无目标可见/伤害前后截图 | `已接入+已回归（数值状态纵切）`；AI/Player 初始与重生 spawn protection 分别以 15/75 原帧消退，USP2 非反射 Unit hit 会按原 Status 公式改变 session HP，dead actor 在 150 后一帧按真实 NodeSpawn 重建。**未完成**：反射/反弹、命中特效/声音、比分/经验、任务/胜负、截图对照；因此不能称为完整命中、可见战斗或第一关完成。 |
| Campaign 1 死亡/尸体生命周期 | `Unit.die()` 的 `createCorpse → visible=false → respawnTimer=150`；`Player.as/AI.as` 的 timer→下一帧 spawn；`Unit.unitSpawn()` 的 NodeSpawn、`Status.reset()` 与 75/15 protection；`PhysWorld.createCorpse()` 的 `impulseAll`、Movement、force/sky9/splash/headMult；`PhysActor` 14 部件与第 150 帧销毁 | `tutorial-corpse-runtime.mjs`、`campaign-one-session.mjs`、`tutorial-scene-preview.mjs`；页面把 USP2 的 `Status.damage().died` 接到来源 corpse 集合，让后续 trace 带入该集合，并在 source respawn tick 重置可见 Player | `tutorial-corpse-runtime.test.mjs`、`campaign-one-session.test.mjs`、`tutorial-scene-preview.test.mjs`；RED/GREEN `2bc88a6→fa54799`、`37e61f8→e5ecb65`、`86611a5→b8096f0`、`093fff9→bdd087f`、`4af9c16→fe4a4df` | 页面仅作 800×600/ready/error 加载健康检查；没有原版死亡帧、尸体或击尸截图对照 | `已接入+已回归（状态/生命周期纵切）`；14 个部件的状态和冲量参数已记录、死亡 Unit 停止 Status、尸体在 150 帧移除，actor 下一帧依真实 NodeSpawn 重生且页面不再保留死者旧位置。**未完成**：Box2D 关节/碰撞、PhysActor 可视 Shape、死亡/复活展示、计分经验、音效特效及逐帧差分；不得称为尸体物理、可见战斗、第一关或游戏 1:1 完成。 |
