...
- 30 个战役/挑战 `Stats_Campaign.setMatch()` 配置已由原 AS3 机械提取为浏览器目录数据；Campaign 1 的计时、比分、脚底触发色、子弹环境命中与换枪推进也已从四个原始 AS3 类生成可审计数据。该数据尚未接入引擎，未迁移完整角色、脚本、过场和结算的任务会明确拒绝启动，绝不回退为快速对战；
- `npm run test:coverage` 当前为 **141/141 通过**（99.32% 行、87.56% 分支、94.44% 函数；覆盖率门槛在脚本中强制）。其中包含战役目录与 Campaign 1 脚本源码一致性、Hud 文本字段和 Aimer Flash 矩阵→Canvas 映射回归；这些都不等于战役、动态经验条或完整 HUD 已完成。
