...
| Campaign 1 Unit/Status/Bullet 数值状态 | `Stats_Skills.Init` 22 条 `addSkill`；`Stats_Classes.getAiLevel/getClass`；`Unit.setClass/unitSpawn`；`AI.spawn`；`Status.reset/damage/EnterFrame`；`Bullet_Line_Basic → Bullet.doHitEffect` | `skill-source.mjs`、`tutorial-unit-profile.mjs`、`tutorial-status-damage-runtime.mjs`、`tutorial-bullet-hit-effects.mjs`、`campaign-one-session.mjs`；仅已 spawn actor 有 source `unitInfo/status/gun`，page USP2 命中会调用 `Status.damage` 并在其后推进 `Status.EnterFrame` | `skill-source.test.mjs`、`tutorial-unit-profile.test.mjs`、`tutorial-status-damage-runtime.test.mjs`、`tutorial-bullet-hit-effects.test.mjs`、`campaign-one-session.test.mjs`、`tutorial-scene-preview.test.mjs` | 仅验证页面无加载错误；尚无目标可见/伤害前后截图 | `已接入+已回归（数值状态纵切）`；AI spawn protection 以 15 原帧消退，USP2 非反射 Unit hit 会按原 Status 公式改变 session HP。**未完成**：目标 UnitMC/枪械渲染，`Unit.die`/尸体物理、反射/反弹、血条/命中特效/声音、AI、比分/经验、重生、任务/胜负、截图对照；因此不能称为完整命中、可见战斗或第一关完成。 |
| Campaign 1 死亡/尸体生命周期 | `Unit.die()` 的 `createCorpse → visible=false → respawnTimer=150`；`PhysWorld.createCorpse()` 的 `impulseAll`、Movement、force/sky9/splash/headMult；`PhysActor` 14 部件与第 150 帧销毁 | `tutorial-corpse-runtime.mjs`、`campaign-one-session.mjs`、`tutorial-scene-preview.mjs`；页面把 USP2 的 `Status.damage().died` 接到来源 corpse 集合，并让后续 trace 带入该集合 | `tutorial-corpse-runtime.test.mjs`、`campaign-one-session.test.mjs`、`tutorial-scene-preview.test.mjs`；RED/GREEN `2bc88a6→fa54799`、`37e61f8→e5ecb65`、`86611a5→b8096f0` | 页面仅作 800×600/ready/error 加载健康检查；没有原版死亡帧、尸体或击尸截图对照 | `已接入+已回归（状态/生命周期纵切）`；14 个部件的状态和冲量参数已记录、死亡 Unit 停止 Status、尸体在 150 帧移除。**未完成**：Box2D 关节/碰撞、PhysActor 可视 Shape、死亡/复活展示、AI/敌方 UnitMC、计分经验、音效特效及逐帧差分；不得称为尸体物理、可见战斗、第一关或游戏 1:1 完成。 |
