...
- HUD 1540 的原 `ScoreBar`（1462）和原经验容器（1477）已在其原 800×600 锚点接入；当前 M4 的 `bulletCont`（954）也已按原 `Hud.as` 公式和双轴翻转接入；这仍只是 HUD 的局部，动态文本/枪图/经验状态尚未完成；
- `npm run test:coverage` 当前为 **127/127 通过**（99.08% 行、87.96% 分支、93.69% 函数；覆盖率门槛在脚本中强制）。
