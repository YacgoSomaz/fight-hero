...
- `UnitMC` 使用从原 SWF 直接裁出的完整人物基准帧；完整逐帧矩阵仍待进一步迁移，避免不正确的分件枢轴造成肢体错位；
- 提取的 Foundry `wall` alpha 遮罩统一驱动角色碰撞、子弹阻挡和 AI 视线，并以金属地形前景显示同一份可走几何；无 mask 时保留平台回退，便于单元测试；
...
tests/
  engine.test.mjs  27 个 Node 单元/集成测试
docs/
