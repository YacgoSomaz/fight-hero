...
test('a continuous source-style ramp remains traversable from end to end', () => {
  ...
  assert.ok(p1.x > 1450, 'the actor passes the full slope instead of sticking at its first pixel');
  assert.equal(p1.grounded, true);
  assert.ok(Math.abs(p1.y - floorAt(p1.x)) <= 1, 'the centre foot stays on the source slope');
});
