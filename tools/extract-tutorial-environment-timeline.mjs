...
const TWIPS = 20;
const FIXED = 65536;
const TARGET_SYMBOLS = new Set([1361, 1388]);
...
function parseSprite(body) {
  ...
  for (let offset = 4; offset < body.length;) {
    ...
    if (tag.code === 26) {
      ...
      frames.push({ frame: frames.length + 1, items: frameItems(display) });
    } else if (tag.code === 0) {
      break;
    }
    offset = tag.next;
...
export function extractTutorialEnvironmentTimelines(source) {
  ...
  for (let offset = header.byte; offset < swf.length;) {
    ...
    if (tag.code === 39 && TARGET_SYMBOLS.has(body.readUInt16LE(0))) timelines[body.readUInt16LE(0)] = parseSprite(body);
    if (tag.code === 0) break;
    offset = tag.next;
  }
  if (Object.keys(timelines).length !== TARGET_SYMBOLS.size) throw new Error('original Tutorial environment timeline source is incomplete');
  return timelines;
}
