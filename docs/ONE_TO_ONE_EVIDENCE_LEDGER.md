...
| --- | --- | --- | --- | --- | --- |
| Campaign 1 `unit0` 的 root + M4 | `Stats_Campaign.setMatch()` 的 unit0/skin 7；`UnitMC` symbol 669/449 帧、`EnterFrame()`；M4 501/668 Display List；`Guns.as:setFrame()`；`arm_gun_316.as` callback | `tutorial-actor-bindings.mjs`、`tutorial-actor-playback.mjs`、`tutorial-actor-preview.html`、`tutorial-actor-preview.mjs`；画布只读取原 Shape、root timeline 和 M4 runtime | `tutorial-actor-render-plan.test.mjs`、`tutorial-actor-playback.test.mjs`、`tutorial-actor-preview.test.mjs`、`tutorial-unit-pose-plan.test.mjs`、`tutorial-unit-pose-renderer.test.mjs` | 本地浏览器已加载 800×600 预览并检查无 console error；尚无原 SWF 同输入/同帧叠图 | `已接入+已回归`；当前可由 Tutorial `Movement` 请求原 `run/jump/fall/land/climb` 标签，且仍只使用 unit0、skin 57、M4 source tick。`UnitMC.EnterFrame()` 的 visible `head` 与 `arm1/arm2` x/y 由 `headhold/arm1hold` 覆盖，scale/skew 保持原可见子件值。仍缺真实鼠标瞄准、左键、弹药、弹道、完整碰撞分支、AI、其它动作及原版像素对照，故不可称可玩或完成。 |
| Campaign 1 Tutorial 场景 | `Arena.as:EnterFrame()` 0.7 跟随、边界钳制、背景视差式；`Stats_Campaign.runScripts` 的 frame 0 武器清空和脚底状态；`Movement.as` 的 alpha hitTest、±17/-45 自动起身探针、落地、左右十像素斜坡扫描与 terminal-fall climb；`Wall_tut` 16 帧/2757×1541；`Stats_Maps` 的 `tut` sky/background/Arena frame；Campaign 1 出生点 `{285,705}` | `tutorial-scene-preview.html`、`tutorial-scene-preview.mjs`、`campaign-one-session.mjs`、`tutorial-arena-camera.mjs`、`tutorial-movement.mjs`；每 source tick 消费 runScripts、每脚底 probe 消费 session wall trigger，A/D/S/W 输入进入当前原 `Wall_tut` surface，nextAnim 进入 `UnitMC.goto` 规则 | `campaign-one-session.test.mjs`、`tutorial-arena-camera.test.mjs`、`tutorial-movement.test.mjs`、`tutorial-actor-playback.test.mjs`、`tutorial-scene-preview.test.mjs`、既有 `tutorial-world*.test.mjs` | 本地浏览器实际加载 800×600 场景、原地图三层和 unit0；页面可见初始 M4 已按源 frame 0 隐去；尚无原 SWF 同输入/同帧叠图 | `已接入+已回归（Movement/教程脚本核心纵切）`；已验证原 frame 0 装备状态进入 session、自动起身、贴墙蹲伏、落地、手动跳跃、侧墙小攀爬与左右十像素探针斜坡角。状态 8 的 USP2/瞄准解锁、完整 modifier/降落伞/硬着陆、完整大小攀爬与全部 effect 视觉/声音尚未迁移；也无左键/Guns、伤害、AI、HUD、过场或原版截图差分，不能说第一关或游戏完成。 |
