...
| Campaign 1 Tutorial 场景 | `Arena.as:EnterFrame()` 0.7 跟随、边界钳制、背景视差式；`Movement.as` 的 alpha hitTest、±17/-45 自动起身探针、落地与 terminal-fall climb；`Wall_tut` 16 帧/2757×1541；`Stats_Maps` 的 `tut` sky/background/Arena frame；Campaign 1 出生点 `{285,705}` | `tutorial-scene-preview.html`、`tutorial-scene-preview.mjs`、`tutorial-arena-camera.mjs`、`tutorial-movement.mjs`；逐帧绘制原 Sky/Background/Arena PNG，A/D/S/W 输入进入当前原 `Wall_tut` surface，nextAnim 进入 `UnitMC.goto` 规则 | `tutorial-arena-camera.test.mjs`、`tutorial-movement.test.mjs`、`tutorial-actor-playback.test.mjs`、`tutorial-scene-preview.test.mjs`、既有 `tutorial-world*.test.mjs` | 本地浏览器实际加载 800×600 场景、原地图三层和 unit0；尚无原 SWF 同输入/同帧叠图 | `已接入+已回归（Movement 核心纵切）`；已验证自动起身、贴墙蹲伏、落地、手动跳跃与侧墙小攀爬的源探针/状态转场。还没有完整 `Movement` 倾斜/修改器/降落伞及全分支对照，也无鼠标/左键、Guns、伤害、AI、HUD、过场或战役触发；不能说第一关或游戏完成。 |
