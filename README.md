...
- HUD 1540 的原 `ScoreBar`（1462）和原经验容器（1477）已在其原 800×600 锚点接入；这只是 HUD 的首批静态部件，动态状态仍未完成；
- `npm run test:coverage` 当前为 **124/124 通过**（99.08% 行、88.18% 分支、93.61% 函数；覆盖率门槛在脚本中强制）。
