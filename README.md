...
- `UnitMC` 的 449 帧完整人物实例从原 SWF 逐帧裁出并预载为精灵表，按原标签区间播放正/反向跑动、跳跃、坠落、蹲伏与两种攀爬动作；
...
tests/
  engine.test.mjs  31 个 Node 单元/集成测试
docs/
...
当前基线：31/31 通过。每次改变 `engine.mjs`、`camera.mjs` 或 `unit-rig.mjs` 时，先新增会失败的测试，再写实现。
