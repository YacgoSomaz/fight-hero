...
| --- | --- | --- | --- | --- | --- |
| Campaign 1 `unit0` 的 root + M4/USP2 | `Stats_Campaign.setMatch()` 的 unit0/skin 7；`UnitMC` symbol 669/449 帧、`EnterFrame()`；完整 `Stats_Guns.as:addGun`；`Player.as:MouseDown/MouseUp/EnterFrame`；`Guns.as:shoot/EnterFrame`；501/668 Display List；`arm_gun_316.as` callback；`MuzzleFlash_317.as:gotoAndStop(UT.irand(1,totalFrames))` | `gun-source.mjs`（每条原始 gun record 的机械产物）、`tutorial-gun-runtime.mjs`、`tutorial-actor-bindings.mjs`、`tutorial-gun-action-frame.mjs`、`tutorial-actor-playback.mjs`、`tutorial-actor-preview.html`、`tutorial-actor-preview.mjs`；画布只读取原 Shape、root timeline 和原 arm runtime | `gun-source.test.mjs`、`tutorial-gun-runtime.test.mjs`、`tutorial-actor-render-plan.test.mjs`、`tutorial-actor-playback.test.mjs`、`tutorial-actor-preview.test.mjs`、`tutorial-usp2-action-frame.test.mjs`、`tutorial-unit-pose-plan.test.mjs`、`tutorial-unit-pose-renderer.test.mjs`、`usp2-muzzle-runtime-extraction.test.mjs` | 本地浏览器已加载 800×600 预览并检查无 console error；尚无原 SWF 同输入/同帧叠图 | `已接入+已回归（局部武器纵切）`；当前可由 Tutorial `Movement` 请求原 `run/jump/fall/land/climb` 标签，并可对 unit0 skin 57 回放 M4 与 Campaign 状态 8 的 USP2（pistol 2、fire 3–8、reload 9–37，USP Sprite frame 2）。USP2 现按原 `mDown`、non-auto `shotPressed`、`shootDelay=.25*30` 的 AS3 `uint` 写入及同 tick `Guns.EnterFrame` 递减、`noAmmo=true` 规则开火；首发 tick 保留 394 的一次原 1..8 随机帧和原 arm 局部矩阵。仍缺真实鼠标瞄准、`makeBullet`、弹道/伤害/音频、完整碰撞分支、AI、其它武器/动作及原版像素对照，故不可称可玩或完成。 |
