...
若要真正实现第一关，先读 [CAMPAIGN_ONE_EXECUTION_SPEC.md](CAMPAIGN_ONE_EXECUTION_SPEC.md)。该文件将完整原包中的关卡快照、actor 顺序、逐 tick 调度、`sn/fc` 状态机、wall/door/elevator/arrow、枪械/子弹/伤害/TDM 结算链及当前硬不可达点写为实施与验收契约；它明确说明当前试玩页不能通关。
...
- `Unit.setJug()` 的第二帧也已直连：shape 686 原始 PNG 使用其 source RECT 注册范围 `(-17.45,-101.85)→(20.8,-89.1)`，并且 684/685 同步采用第二帧的 0.25/0.45 像素矩阵偏移；它只在 `isJug` 为真时出现在文本 depth 9 之上。
- TDD：`13ecb63→6a08394`（源 HUD 计划）与 `99e68d9→f9f3170`（场景承载）为 RED→GREEN；`0fa60dd→629502f` 使场景帧循环错误可见；`98a6659→d139c0b` 锁定 684/685 的原字体、矩阵、文字、颜色和拒绝替代行为；`08bb49e→bc6b7dc` 锁定 682 图标、`Unit.setClass()` 的 icon 字段与原 depth 顺序；`7121fb2→0df9225` 锁定 `setJug()` 的 frame 2 marker 与文本矩阵。
- 严格边界：已接入 **bar_hp/bar_hurt/icon/txt_name/txt_level/Jug marker**，但仍没有底部职业/生命/弹药/经验动态 UI，更没有原版截图对照；当前内嵌浏览器无法渲染 Canvas 供截图验证，真实 Chrome 连接在本机不可用，故这项没有视觉验收结论。因此仍不得称“头顶 HUD 完成”或“Campaign 1 可玩”。
...
- 原始证据：`Unit.die()` 在 `!gameEnded` 的普通异队击杀分支先让死者 `Score.addDeath()`，再让击杀者 `Score.addKill()`、按 `headMult` 增加 `headshots`、按死者 `unitInfo.num` 增加 `killedN`；人类击杀的奖赏为 `ceil(min(Stats_Classes.getUnitExp(attacker.level + 3), Stats_Classes.getUnitExp(target.level)))`。`Stats_Classes.getUnitExp(level)` 为 `4 + level * 1.4`。友军伤害分支已用新包的 `rabcdasm/war-heroes-4399-0/Unit.class.asasm` 的 `Unit.die` 裁决：`getlocal0`（死亡目标）→ `score` → `addBetrayal()`，故原版扣的是死亡目标的 `betrayals`，不可按常规直觉改成击杀者。`Score.updateScore()` 在 TDM 写 `pscore = kills - suicides - betrayals`，`MatchSettings.updateScores()` 汇总 team 1/2、在 score limit 截断并通知 HUD。`Hud.addExp()` 将奖励写入 `SD.classSaves[class].funds/exp`，用 `level² * 3 + 40` 判断升级，达到阈值时清零 exp 并升一级。
