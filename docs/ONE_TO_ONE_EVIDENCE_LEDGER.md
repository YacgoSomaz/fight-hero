...
| 子项 | 原始证据 | 当前网页承载 | 自动证据 | 当前状态 / 必须补项 |
| --- | --- | --- | --- | --- |
| AI 目标/视线、行动与瞄准 | `AI.as`；每 1–12 帧错峰扫描，20px wall line-of-sight，450/枪射程限制；AI key/jump → `Movement.as` → `UnitMC.goto`；`Unit.as` arm/head rotation 段 | `tutorial-ai-runtime.mjs`、`campaign-one-session.mjs:advanceCampaignOneSessionAiMovement()`、`tutorial-aim-runtime.mjs:deriveTutorialUnitAim()`、`tutorial-scene-preview.mjs`；不调用旧通用 `engine.mjs` AI | `tutorial-ai-runtime.test.mjs`、`tutorial-aim-runtime.test.mjs`、`campaign-one-session.test.mjs`、`tutorial-scene-preview.test.mjs`；RED/GREEN `9450a59→570cc91`、`b79626c→d181cce`、`48e3164→fd0c630`、`f2fb52a→bb496ea`、`b579105→1306caf`、`1ec1291→478da01` | `已接入+已回归（决策、原 Movement、UnitMC 行动和 arm/head 瞄准纵切）`；**未完成**：`aiShouldShoot→Guns.shoot`、弹道/伤害/重生、可见血条及原版输入/截图差分，不能称完整 AI。 |
