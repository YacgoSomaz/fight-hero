...
本地入口：<http://127.0.0.1:4173>。当前基线：`npm test` **122/122 通过**，覆盖率为 **99.08% 行、88.19% 分支、93.61% 函数**，高于 80% 门槛。
...
**仍未完成且不能误报**：1431 的狙击 frame 2、反射状态、非 M4 枪械、其他兵种/等级/技能倍率、以及和原 800×600 录屏的逐帧叠图都没有完成。完整 Hud 1540 仍是独立阻断项；还必须先修正原 SWF 800×600 与现有 1280×720 的坐标/纵横比契约，才可做像素级 HUD 对照。
