...
- `npm run test:coverage` 当前为 **163/163 通过**（99.37% 行、87.50% 分支、95.51% 函数；覆盖率门槛在脚本中强制）。其中包含战役目录与 Campaign 1 脚本源码一致性、Hud 文本字段和 Aimer Flash 矩阵→Canvas 映射回归；这些都不等于战役、动态经验条或完整 HUD 已完成。
