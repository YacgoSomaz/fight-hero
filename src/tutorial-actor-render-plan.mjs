import { tutorialM4ActionFrame, createTutorialUnitPoseAtAction } from './tutorial-m4-action-frame.mjs';
...
export function createTutorialActorRenderPlan({ actor, rootState, actionState, unitTimeline, m4Runtime } = {}) {
  ...
  const arm = tutorialM4ActionFrame(m4Runtime, actionState.label, actionState.index);
  return {
    actorId: actor.id,
    ...
    arm,
    pose: createTutorialUnitPoseAtAction({ rootFrame, runtime: m4Runtime, actionLabel: actionState.label, actionIndex: actionState.index, skinFrame: actor.skinFrame }),
  };
}
