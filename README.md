...
- 30 个战役/挑战 `Stats_Campaign.setMatch()` 配置已由原 AS3 机械提取为浏览器目录数据；未迁移完整角色、脚本、过场和结算的任务会明确拒绝启动，绝不回退为快速对战；
- `npm run test:coverage` 当前为 **138/138 通过**（99.35% 行、87.70% 分支、94.12% 函数；覆盖率门槛在脚本中强制）。其中包含战役目录源码一致性、Hud 文本字段和 Aimer Flash 矩阵→Canvas 映射回归；这些都不等于战役、动态经验条或完整 HUD 已完成。
