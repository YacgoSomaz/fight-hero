...
- `npm run test:coverage` 当前为 **132/132 通过**（99.09% 行、87.34% 分支、93.81% 函数；覆盖率门槛在脚本中强制）。其中包含原 `Hud.addExp` 的等级公式/50 级例外、Hud 文本字段和 Aimer Flash 矩阵→Canvas 映射回归；这些都不等于动态经验条或完整 HUD 已完成。
